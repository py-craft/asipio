...
async def run_subscription(peer, user, duration):
    subscription = await peer.subscribe(
        ...
        to_details=aiovoip.Contact.from_header('sip:{}@{}:{}'.format(
...
async def start(app, protocol, target, duration):
    ...
    registration = Registration(
        ...
        to_details=aiovoip.Contact.from_header('sip:{}@{}:{}'.format(
            ...
            sip_config['srv_port'])),
        contact_details=aiovoip.Contact.from_header('sip:{}@{}:{}'.format(
            sip_config['user'], sip_config['local_host'],
            sip_config['local_port'])),
        password=sip_config['pwd']
    )
...
def main():
    ...
    if args.protocol == 'udp':
        client = start(app, aiovoip.UDP, args.target, args.duration)
    elif args.protocol == 'tcp':
        client = start(app, aiovoip.TCP, args.target, args.duration)
    elif args.protocol == 'ws':
        client = start(app, aiovoip.WS, args.target, args.duration)
    else:
